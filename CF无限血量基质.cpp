...
int main(int argc, char **argv)
{
	...
	// 读取游戏进程
	int ipid = getPID("com.netease.party.bazhang");
    char lj[64];
	...
	// 读取基址
	puts("\n开始基址搜索");
	char mname[] = "libclient.so:bss";	// 基址入口模块
	//Xa用getXa,Cb用getbss,Cd用getCd
	long int fool = getXa(ipid, mname);
    //64位游戏把lsp64换成lsp64即可
    //0x173F8, 0x90,0x98,0x5C+0x9F0
	long int a1 = lsp64(lsp64(fool + 0x173F8) + 0x90)+ 0x98)+ 0x5C+0x9F0;
	printf("基址搜索完成\n");
    printf("\n地址=%lx\n", a1);
	WriteAddress_DWORD(a1, 50);
	//修改支持所有类型
	puts("\n修改成功\n");
	}
